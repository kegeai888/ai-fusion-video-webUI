 ...
 * <p>
 * 整集替换场次数据。需传入 episode_version 做乐观锁校验。
 */
@Component
...
public class SaveScriptSceneItemsToolExecutor implements ToolExecutor {
    ...
    public String getToolDescription() {
        return """
                ...
                默认模式（appendMode=false）会清空该集所有旧场次后写入新数据；追加模式（appendMode=true）在已有场次后追加。

                ## 分批调用规则（必须遵守！）
                - 每次调用 scenes 数组最多传入 3 个场次
                - 如果一集有超过 3 个场次，必须分多次调用：
                  - 第一次调用：传入前 3 个场次，appendMode 不传或设为 false（会清空旧数据）
                  - 第二次及之后：传入后续 1-3 个场次，appendMode 必须设为 true（追加模式）
                - 例如一集有 5 个场次，需要调用 2 次：
                  - 第1次：scenes=[场次1, 场次2, 场次3]，appendMode=false
                  - 第2次：scenes=[场次4, 场次5]，appendMode=true
        ...
        return """
                {
                    ...
                    "properties": {
                        ...
                        "appendMode": {
                            "type": "boolean",
                            "description": "追加模式。true=在已有场次后追加，不删除旧数据；false或不传=整集替换（默认）。分批写入时第二次及之后调用必须设为true"
                        }
                    },
    ...
    public String execute(String toolInput, ToolExecutionContext context) {
        try {
            ...
            Boolean appendMode = params.getBool("appendMode", false);
            ...
            scriptService.batchSaveSceneItems(episodeId, episodeVersion, sceneItems, appendMode);

            JSONObject resultObj = JSONUtil.createObj()
                    .set("episodeId", episodeId)
                    .set("sceneCount", sceneItems.size())
                    .set("message", String.format("成功写入 %d 个场次", sceneItems.size()));
